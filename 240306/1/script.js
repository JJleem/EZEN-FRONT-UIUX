...
//2) 일반객체
//장점 : key, value 값을 가지고있는 프로퍼티
//데이터 체계적으로 보관,관리,사용
//단점 : for..in 문 사용할수있긴하나 => 객체안에있는 value값 만을 찾아와서 직접적으로 반복문을 돌릴 수 있는 방법 x
//단점 : 객체안에 구성되어있는 아이템들은 배열처럼 인덱스번호를 할당받지 못한다!!

//3) 배열객체
//장점: 배열안에 포함되어있는 아이템들이 고유의 인덱스값을 가지고있으며, for문을 활용한 반복문으로 업무의 효율성 극대화
//단점: 객체처럼 key & value 형태로 구성된 체계적인 데이터 관리 x

// Solution : 일반객체 & 배열객체의 장점만 모아서 하나로 만들어주면 어떨까? => map 객체
...
console.log(myCup.has("coldddor"));
myCup.delete("type");
// myCup.clear();
console.log(myCup);
